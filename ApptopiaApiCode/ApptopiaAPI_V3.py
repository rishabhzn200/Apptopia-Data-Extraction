...
token = "eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9..4Wh8xUZrltrNX0vIJtd5kbGlGe7cyY8X_dnNd8Zy2_Q_nOT1H_Mexz8YkbeeFAgoU7oYVssFXkSbfpPFBAWnHw"
...
def get_estimates(appid, store='itunes_connect', country='US', start_date='2015-05-01', end_date='2015-11-01'):
    '''
...
def get_estimates_group(appids, store='itunes_connect', country='US', start_date='2016-11-01', end_date='2017-10-31'):
    '''
    ...
    # Combine the 50 ids for request
    appid_url_list = ''
    if len(appids) == 1:
        appid_url_list = appids[0]
    else:
        appid_url_list = '&id%5B%5D='.join(appids)

    # print(f"\nURL = {appid_url_list}\n\n")
    # print(f"\n/api/{store}/estimates?id%5B%5D={appid_url_list}&country_iso={country}&date_from=\n\n")
    ...
    for index in range(0, len(dates), 180)[1:]:
        ...
        # Find estimate
        conn.request("GET",
                     f"/api/{store}/estimates?id%5B%5D={appid_url_list}&country_iso={country}&date_from={s}&date_to={e}",
                     payload, headers)
        ...
        try:
            ...
            with open('./LiteralEvalError.txt', 'a') as f:
                f.write('\n\n_________________ERROR while using literal eval _________________________\n')
                f.write(f'Status={res.status}. Not adding this data to main file. Load manually later.')
                f.write(f'\nID = {appid_url_list}\tcountry = {country}\tst_date={s}\tend_date={e}\n')
                f.write(f'Data={dataStr}\n')
